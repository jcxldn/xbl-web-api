...
class Catalog(BlueprintProvider, LoopbackRequestProvider):
    async def __get_product_from_alt_id(
        self, id, type: AlternateIdType, fields: FieldsTemplate
    ):
        return await self.xbl_client.catalog.get_product_from_alternate_id(
            id, type, fields
        )

    async def __boxart_callback_on_product_lookup_finish(
        self, imagePurpose: str, lookupRes: ClientResponse
    ):
        # Check status code
        if lookupRes.status == 200:
            ...
            try:
                ...
                dat = (
                    list(
                        filter(lambda i: i["imagePurpose"] == imagePurpose, image_array)
                    )
                )[0]
            except IndexError:
                ...
                try:
                    ...
                    if imagePurpose == "BoxArt":
                        ...
                        dat = (
                            list(
                                filter(
                                    lambda i: i["imagePurpose"] == "Poster", image_array
                                )
                            )
                        )[0]
                    # KeyArt -> Tile
                    if imagePurpose == "BrandedKeyArt":
                        tileList = list(
                            filter(lambda i: i["imagePurpose"] == "Tile", image_array)
                        )
            ...
            url = (
                dat["uri"].replace("//", "https://")
                if dat["uri"].startswith("//")
                else dat["uri"]
            )
    ...
    async def __boxart_make_loopback_request(self, image, lookupType, id):
        def imageTypes(x):
            return {"boxart": "BoxArt", "keyart": "BrandedKeyArt"}[x]
        ...
        return await self.get(
            "http://localhost:%i/catalog/lookup/%s/%s/browse"
            % (self.xbl_client._xbl_web_api_current_port, lookupType, id),
            partial(self.__boxart_callback_on_product_lookup_finish, imagePurpose),
        )
    ...
    def routes(self):
        ...
        async def lookup_bigid(id, fields):
            fieldsEnum = (
                FieldsTemplate.BROWSE if fields == "browse" else FieldsTemplate.DETAILS
            )
            return await self.xbl_client.catalog.get_products([id], fieldsEnum)

        # ---------- ALTERNATE lookup via pid / titleid/ pfn ----------
        @self.openXboxRoute(
            "/lookup/legacyproductid/<int:id>/<any(browse, details):fields>"
        )
        async def lookup_alternateid_legacy(id, fields):
            fieldsEnum = (
                FieldsTemplate.BROWSE if fields == "browse" else FieldsTemplate.DETAILS
            )
            return await self.__get_product_from_alt_id(
                id, AlternateIdType.LEGACY_XBOX_PRODUCT_ID, fieldsEnum
            )
        ...
        async def lookup_alternateid_titleid(id, fields):
            ...
            return await self.__get_product_from_alt_id(
                id, AlternateIdType.XBOX_TITLE_ID, fieldsEnum
            )
        ...
        async def lookup_alternateid_pfn(id, fields):
            ...
            return await self.__get_product_from_alt_id(
                id, AlternateIdType.PACKAGE_FAMILY_NAME, fieldsEnum
            )
        ...
        @self.xbl_decorator.cachedRoute(
            "/img/<any(boxart, keyart):image>/<any(bigid, legacyproductid, titleid, pfn):lookupType>/<id>"
        )
