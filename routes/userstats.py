                ...
                if (xuidRes.status == 200):
                    ...
                    async def on_stats_request_finish(statsRes: ClientResponse):
                        if (statsRes.status == 200):
                            ...
                            response.headers.add_header("x-upstream-queried-at", statsRes.headers.get("x-queried-at"))
                            return response
                        else:
                            response = jsonify({"error": "error getting stats", "code": statsRes.status})
                            response.status_code = 404
                            return response
                    # Make an async request to our own xuid titleid stats endpoint using a callback
                    return await self.get("http://localhost:%i/userstats/xuid/%s/titleid/%s" % (self.xbl_client._xbl_web_api_current_port, xuid, titleid), on_stats_request_finish)
                # on_xuid_lookup_finish -> status code is not 404
                else:
                    response = jsonify({"error": "could not resolve gamertag", "code": xuidRes.status})
                    response.status_code = 404
                    return response
