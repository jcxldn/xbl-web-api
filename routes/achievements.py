...
class Achievements(BlueprintProvider):
    def routes(self):
        ...
        async def recent1(xuid, continuationToken):
            return await self.xbl_client.achievements.get_achievements_xboxone_recent_progress_and_info(xuid, extra_params = { "continuationToken": continuationToken })
        
        @self.openXboxRoute("/360/recent/<int:xuid>") # 500 when using continuationtoken
        ...
        async def titleprogress360all(xuid, titleid, continuationToken):
            return await self.xbl_client.achievements.get_achievements_xbox360_all(xuid, titleid, extra_params = {"continuationToken": continuationToken })
        ...
        async def titleprogress360earned(xuid, titleid, continuationToken):
            return await self.xbl_client.achievements.get_achievements_xbox360_earned(xuid, titleid, extra_params = {"continuationToken": continuationToken })

        # ---------- Achievement details for user / scid / achievementID ----------
        @self.openXboxRoute("/1/titleprogress/detail/<int:xuid>/<scid>/<int:achievementid>") # continuationToken does nothing (not required anyways? - only one achievement returned)
